...
case class Request(amendCorrespondenceAddressRequest: ReimbursementClaimsRequest)

object Request {
  implicit val format: OFormat[Request] = Json.format[Request]
}

case class ReimbursementClaimsRequest(requestCommon: RequestCommon, requestDetail: RequestDetail)

object ReimbursementClaimsRequest {
  implicit val format: OFormat[ReimbursementClaimsRequest] = Json.format[ReimbursementClaimsRequest]
}
...
case class RequestDetail(eori: String)
